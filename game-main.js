...
C.GAME_ROUND_DURATION_MS = 4 * 60 * 1000;
C.NEW_GAME_COUNTDOWN_MS = 30 * 1000;
...
let currentActualOrientation = ""; // To store current actual orientation

function getActualScreenOrientation() {
...
function checkAndDisplayRotateOverlay() {
  const overlay = document.getElementById("rotate-overlay");
  if (!overlay) return;

  currentActualOrientation = getActualScreenOrientation();
  if (currentActualOrientation.includes("portrait")) {
    console.log("Device is in Portrait, showing overlay.");
    overlay.style.display = "flex";
  } else {
    console.log("Device is in Landscape, hiding overlay.");
...
async function attemptLockLandscape() {
  ...
  try {
    ...
    console.log("Screen orientation lock to landscape attempted/successful.");
    isLockedToLandscape = true;
    return true;
  } catch (error) {
    console.error("Error attempting to lock screen to landscape:", error);
    isLockedToLandscape = false;
    return false;
  }
}
...
async function requestFullscreenAndLockLandscape() {
  ...
  console.log("Now attempting to lock landscape.");
  await attemptLockLandscape();
  checkAndDisplayRotateOverlay(); // Check orientation after attempts
}
...
class GameModel extends Multisynq.Model {
  ...
  viewJoined(payload) {
    ...
    if (typeof payload === "string") {
      viewId = payload;
    } else if (payload && typeof payload.viewId === "string") {
      viewId = payload.viewId;
      viewData = payload.viewData || {};
    } else {
      console.error("[GameModel] viewJoined: UNEXPECTED payload:", payload);
      return;
    }
    if (!viewId) {
      console.error("[GameModel] viewJoined: viewId undefined.");
      return;
    }
    ...
    if (colorIndex >= 0 && colorIndex < PLAYER_COLORS.length)
      this.playerColorsInUse[colorIndex] = true;
    ...
    try {
      ...
      this.publish(this.id, "player-added", {
      ...
      console.error(
        `[GameModel] Error creating PlayerBallModel for ${viewId}:`,
        e,
      );
    }
  }
  ...
  viewExited(payload) {
    ...
    if (typeof payload === "string") viewId = payload;
      ...
      console.error("[GameModel] viewExited: UNEXPECTED payload:", payload);
      return;
    }
    ...
    if (player) {
      ...
      this.publish(this.id, "player-removed", {
        viewId: player.viewId,
        id: player.id,
      });
      player.destroy();
      ...
      if (
        this.players.size < 2 &&
        (this.gameState === "playing" || this.gameState === "countdown")
      ) {
  ...
  startGame() {
    console.log(
      "[MODEL.startGame] Starting game. Resetting player scores and states.",
    );
    ...
    this.players.forEach((p, viewId) => {
      ...
      console.log(
        `[MODEL.startGame] Player ${viewId}: score reset from ${oldScore} to 0.`,
      );
    });
    this.assignNewIt();
    this.publish(this.id, "game-state-changed", {
  ...
  endGame(winnerFromLeave = null) {
    ...
    this.winnerViewIds =
      determinedWinnerViewIds.length > 0 ? determinedWinnerViewIds : null;
    console.log("[MODEL.endGame] Winner(s) determined:", this.winnerViewIds);

    this.publish(this.id, "game-state-changed", {
      state: this.gameState,
      winners: this.winnerViewIds,
    });
  ...
  assignNewIt() {
    ...
    const newItPlayer =
      playerArray[Math.floor(this.random() * playerArray.length)];
  ...
  tick() {
    ...
    this.players.forEach((player) => {
      ...
      for (let i = this.blueCubes.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          player.score++;
          this.publish(this.id, "score-updated", {
            viewId: player.viewId,
            score: player.score,
          });
    ...
    if (
      this.blueCubes.length < C.MAX_BLUE_CUBES &&
      this.random() < C.BLUE_CUBE_SPAWN_CHANCE
    ) {
      this.spawnBlueCube();
    }
    this.future(C.MODEL_TICK_RATE).tick();
  ...
  handleTag(itPlayer, taggedPlayer) {
    console.log(
      `[MODEL.handleTag] CALLED: It=${itPlayer.viewId} tagged ${taggedPlayer.viewId}. Time: ${this.now()}`,
    );
    ...
    this.publish(this.id, "player-tagged", {
      oldItViewId: itPlayer.viewId,
      newItViewId: taggedPlayer.viewId,
      itPlayerScore: itPlayer.score,
      taggedPlayerScore: taggedPlayer.score,
    });
    this.publish(this.id, "it-changed", {
      newItViewId: taggedPlayer.viewId,
      oldItViewId: itPlayer.viewId,
    });
  }
  ...
  spawnInitialBlueCubes() {
    for (let i = 0; i < C.MAX_BLUE_CUBES; i++) this.spawnBlueCube();
  }
  ...
  isCellOccupiedByCube(gridX, gridY) {
    return this.blueCubes.some((cube) => cube.x === gridX && cube.y === gridY);
  }
}
...
class PlayerBallModel extends Multisynq.Model {
  ...
  resetPositionToSpawn() {
    ...
    this.publish(this.id, "teleported", {
      gridX: this.gridX,
      gridY: this.gridY,
  ...
  update() {
    ...
    if (this.gridX !== this.targetGridX || this.gridY !== this.targetGridY) {
      ...
      let progress = Math.min(
        1.0,
        elapsedTimeInSegment / timeToCompleteSegment,
      );
  ...
  decideNextMove() {
    ...
    const currentWallInfo = this.gameModel.getCellWallInfo(
      this.gridX,
      this.gridY,
    );
    let possibleExits = [];
    ...
    if (!chosenMove) {
      ...
      const nonReverseExits = possibleExits.filter(
        (exit) => exit.newDir !== (this.direction + 2) % 4,
      );
      if (nonReverseExits.length > 0) {
        const forwardExit = nonReverseExits.find(
          (exit) => exit.newDir === this.direction,
        );
        if (forwardExit) {
          chosenMove = forwardExit;
        } else {
          chosenMove =
            nonReverseExits[Math.floor(this.random() * nonReverseExits.length)];
        }
    ...
    if (chosenMove) {
      if (
        this.gridX + chosenMove.dx !== this.gridX ||
        this.gridY + chosenMove.dy !== this.gridY ||
        this.direction !== chosenMove.newDir
      ) {
        this.setTarget(
          this.gridX + chosenMove.dx,
          this.gridY + chosenMove.dy,
          chosenMove.newDir,
        );
      } else {
        if (
  ...
  bounce(isPlayerCollision = false, otherPlayerNewDir = null) {
    ...
    if (possibleBounceDirs.length > 0) {
      chosenBounceDir =
        possibleBounceDirs[
          Math.floor(this.random() * possibleBounceDirs.length)
        ];
    } else {
      possibleBounceDirs = [];
      for (let i = 0; i < 4; i++) {
    ...
    if (chosenBounceDir !== null) {
      ...
      this.setTarget(this.gridX, this.gridY, (this.direction + 2) % 4);
    }
  }

  hasWallInDirection(cellWallInfo, dir) {
    if (dir === 0 && cellWallInfo & WALL_NORTH) return true;
    if (dir === 1 && cellWallInfo & WALL_EAST) return true;
    if (dir === 2 && cellWallInfo & WALL_SOUTH) return true;
    if (dir === 3 && cellWallInfo & WALL_WEST) return true;
    return false;
  }
  ...
  addChalkMark(gridX, gridY) {
    ...
    this.publish(this.id, "chalk-update", {
      trail: this.chalkTrail.map((t) => ({ ...t })),
    });
  }
}
...
class GameView extends Multisynq.View {
  constructor(model) {
    ...
    this.blueCubeObjects = new Map();
    this.chalkTrailObjects = new Map();
    this.viewLerpFactor = 0.53;
    this.currentCameraOffset = new THREE.Vector3(0, 4, 7);

    this.initThree();
    this.initControls(); // Call initControls early

    if (this.isMobileDevice()) {
      console.log(
        "Mobile device detected. Setting up fullscreen and landscape lock.",
      );
      checkAndDisplayRotateOverlay(); // Initial check
      ...
      if (screen.orientation) {
        screen.orientation.addEventListener("change", () => {
          ...
          currentActualOrientation = screen.orientation.type;
          if (
            isLockedToLandscape &&
            !currentActualOrientation.includes("landscape")
          ) {
            console.log(
              "Orientation changed from landscape, attempting to re-lock.",
            );
            attemptLockLandscape().then(() => checkAndDisplayRotateOverlay());
          } else {
            checkAndDisplayRotateOverlay();
          }
        });
      } else {
      ...
      const handleFirstInteraction = async () => {
        console.log(
          "First user interaction on mobile, requesting fullscreen and landscape lock.",
        );
        await requestFullscreenAndLockLandscape();
        // Listeners are {once: true} so they remove themselves
      };
    ...
    this.subscribe(
      this.gameModel.id,
      "player-added",
      this.onPlayerAdded.bind(this),
    );
    this.subscribe(
      this.gameModel.id,
      "player-removed",
      this.onPlayerRemoved.bind(this),
    );
    this.subscribe(
      this.gameModel.id,
      "it-changed",
      this.onItChanged.bind(this),
    );
    this.subscribe(
      this.gameModel.id,
      "player-tagged",
      this.onPlayerTagged.bind(this),
    );
    this.subscribe(
      this.gameModel.id,
      "score-updated",
      this.onScoreUpdated.bind(this),
    );
    this.subscribe(
      this.gameModel.id,
      "cube-spawned",
      this.onCubeSpawned.bind(this),
    );
    this.subscribe(
      this.gameModel.id,
      "cube-collected",
      this.onCubeCollected.bind(this),
    );
    this.subscribe(
      this.gameModel.id,
      "game-state-changed",
    ...
    this.onGameStateChanged({
      ...
      endTime: this.gameModel.gameEndTime,
      nextGameTime: this.gameModel.nextGameStartTime,
      winners: this.gameModel.winnerViewIds,
    });
    if (
      this.gameModel.gameState === "playing" ||
      this.gameModel.gameState === "countdown"
    ) {
    ...
    // Multisynq.App.makeWidgetDock(); // Moved to global scope before Session.join
  }
  ...
  initThree() {
    ...
    window.addEventListener(
      ...
      () => {
        ...
        this.renderer.setSize(window.innerWidth, window.innerHeight);
        if (this.isMobileDevice()) checkAndDisplayRotateOverlay(); // Re-check on resize
      },
      false,
    );
  }
  ...
  updateLerpFactorDisplay() {
    document.getElementById("lerpFactorDisplay").textContent =
      `View LERP: ${this.viewLerpFactor.toFixed(2)}`;
  }
  ...
  onPlayerAdded(playerData) {
    let pModelInstance;
    let modelId;
    if (playerData instanceof PlayerBallModel) {
      pModelInstance = playerData;
      modelId = pModelInstance.id;
    } else {
      pModelInstance = this.gameModel.players.get(playerData.viewId);
      modelId = playerData.id;
    }
    ...
    const initialWorldPos = this.gridToWorld(
      pModelInstance.gridX,
      pModelInstance.gridY,
    );
    playerGroup.position.copy(initialWorldPos);
    const initialQuaternion = new THREE.Quaternion();
    initialQuaternion.setFromAxisAngle(
      new THREE.Vector3(0, 1, 0),
      this.directionToAngle(pModelInstance.direction),
    );
    playerGroup.quaternion.copy(initialQuaternion);
    playerGroup.userData = {
      modelId: modelId,
      viewId: pModelInstance.viewId,
      pModel: pModelInstance,
  ...
  subscribeToPlayerModelEvents(playerModel) {
    ...
    this.subscribe(playerModel.id, "target-set", (data) =>
      this.handlePlayerTargetSet(playerModel.viewId, data),
    );
    this.subscribe(playerModel.id, "moved", (data) =>
      this.handlePlayerLogicalMoveComplete(playerModel.viewId, data),
    );
    this.subscribe(
      playerModel.id,
      { event: "moving", handling: "oncePerFrame" },
      (data) => this.handlePlayerVisualProgress(playerModel.viewId, data),
    );
    this.subscribe(playerModel.id, "chalk-update", (data) =>
      this.updateChalkTrail(playerModel.id, data.trail, playerModel.color),
    );
    this.subscribe(playerModel.viewId, "immunity-ended", () =>
      this.handleImmunityChange(playerModel.viewId, false),
    );
    this.subscribe(playerModel.id, "teleported", (data) =>
      this.handlePlayerTeleport(playerModel.viewId, data),
    );
  }
  ...
  handlePlayerTargetSet(viewId, data) {
    ...
    if (!playerObj) return;
    playerObj.userData.visualLerpStartWorldPos = this.gridToWorld(
      data.oldGridX,
      data.oldGridY,
    );
    playerObj.userData.visualLerpEndWorldPos = this.gridToWorld(
      data.targetGridX,
      data.targetGridY,
    );
    ...
    qTarget.setFromAxisAngle(
      new THREE.Vector3(0, 1, 0),
      this.directionToAngle(data.direction),
    );
    ...
    playerObj.userData.animationNextVisualPosition.copy(
    ...
    playerObj.userData.animationNextVisualQuaternion.copy(
      playerObj.userData.visualLerpStartQuaternion,
    );
  ...
  handlePlayerVisualProgress(viewId, data) {
    ...
    if (
      playerObj.userData.visualLerpStartQuaternion &&
      playerObj.userData.visualTargetQuaternion
    ) {
      if (!playerObj.userData.animationNextVisualQuaternion)
  ...
  onPlayerRemoved(data) {
    ...
    if (playerObj) {
      ...
      this.playerObjects.delete(data.id);
      this.clearChalkTrail(data.id);
      this.chalkTrailObjects.delete(data.id);
    }
  }
  ...
  updatePlayerVisualState(viewId) {
    ...
    if (
      !playerObj ||
      !playerObj.userData.pModel ||
      playerObj.children.length === 0
    )
      return;
    ...
    if (newOpacity < 1.0) mainMesh.material.transparent = true;
    if (
      mainMesh.material.opacity !== newOpacity ||
      mainMesh.material.emissive.getHex() !== newEmissive
    ) {
      mainMesh.material.opacity = newOpacity;
  ...
  onItChanged(data) {
    ...
    if (myPModel) {
      ...
      document.getElementById("statusDisplay").textContent = myPModel.isIt
        ? "You are It!"
        : "Run!";
    }
  }
  ...
  onPlayerTagged(data) {
    ...
    if (oldItPlayerObject && oldItPlayerObject.userData.pModel) {
      ...
      oldItPlayerObject.userData.pModel.isIt = false;
      if (this.viewId === data.oldItViewId)
    ...
    if (newItPlayerObject && newItPlayerObject.userData.pModel) {
      ...
      newItPlayerObject.userData.pModel.isIt = true;
      if (this.viewId === data.newItViewId)
        document.getElementById("scoreDisplay").textContent =
          data.taggedPlayerScore;
      this.updatePlayerVisualState(data.newItViewId);
    }
  }
  ...
  handleImmunityChange(viewId, isImmune) {
    ...
    if (playerObj && playerObj.userData.pModel) {
      playerObj.userData.pModel.isImmune = isImmune;
  ...
  onScoreUpdated(data) {
    ...
    if (playerObj && playerObj.userData.pModel)
      playerObj.userData.pModel.score = data.score;
  ...
  onCubeSpawned(data) {
    ...
    this.blueCubeObjects.set(data.id, cubeMesh);
    this.scene.add(cubeMesh);
  }
  onCubeCollected(data) {
    const cubeObj = this.blueCubeObjects.get(data.cubeId);
    if (cubeObj) {
      cubeObj.geometry.dispose();
      cubeObj.material.dispose();
      this.scene.remove(cubeObj);
      this.blueCubeObjects.delete(data.cubeId);
    }
  }
  onGameStateChanged(data) {
    const gameMessageDiv = document.getElementById("gameMessage");
    const winnerChipsContainer = document.getElementById(
      "winnerChipsContainer",
    );
      ...
      if (winnerViewIds && winnerViewIds.length > 0) {
        ...
        winnerViewIds.forEach((winnerId) => {
          ...
          if (winnerPModel) {
            ...
            chip.style.cssText =
              "display:inline-block; width:20px; height:20px; border-radius:50%; background-color:#" +
              winnerPModel.color.toString(16).padStart(6, "0") +
              "; border:1px solid white; vertical-align:middle;";
        ...
        messageTextElement.textContent =
          (winnerViewIds.length > 1 ? " Draw! " : " ") + baseMessage;
      } else {
        messageTextElement.textContent =
          (this.gameModel.players.size > 0
            ? "No one scored. "
            : "No winner. ") + baseMessage;
      }
      ...
      case "countdown":
        ...
        if (data.previousWinners && data.previousWinners.length > 0)
          displayWinnerChips(
            ...
            "won the last round! " + baseCountdownMessage,
          );
        else {
  ...
  update() {
    this.playerObjects.forEach((playerObj) => {
      if (playerObj.userData.animationNextVisualPosition)
        playerObj.position.lerp(
          playerObj.userData.animationNextVisualPosition,
          this.viewLerpFactor,
        );
      if (playerObj.userData.animationNextVisualQuaternion)
        playerObj.quaternion.slerp(
          playerObj.userData.animationNextVisualQuaternion,
          this.viewLerpFactor,
        );
    });
    ...
    if (myPlayerObj && myPlayerObj.userData.pModel) {
      ...
      const camDist = 7;
      const camHeight = 4;
      if (playerModel.direction === 0)
        desiredCamOffset.set(0, camHeight, camDist);
      else if (playerModel.direction === 1)
        desiredCamOffset.set(-camDist, camHeight, 0);
      else if (playerModel.direction === 2)
        desiredCamOffset.set(0, camHeight, -camDist);
      else desiredCamOffset.set(camDist, camHeight, 0);
      if (!this.currentCameraOffset)
        this.currentCameraOffset = desiredCamOffset.clone();
      else this.currentCameraOffset.lerp(desiredCamOffset, 0.03);
      const targetCamPos = new THREE.Vector3();
      targetCamPos.copy(myPlayerObj.position).add(this.currentCameraOffset);
      this.camera.position.lerp(targetCamPos, 0.08);
      this.camera.lookAt(
        myPlayerObj.position.x,
        myPlayerObj.position.y + 0.2,
        myPlayerObj.position.z,
      );
    } else {
  ...
  detach() {
    // Note: Add cleanup for global event listeners (orientationchange, resize, pointerdown, touchstart) if they were not {once:true}
    // or if the GameView instance might be detached and recreated multiple times without a full page reload.
    // For now, the {once:true} on pointer/touch and the standard screen.orientation.onchange should be mostly fine.
    if (this.renderer) {
    ...
    this.blueCubeObjects.forEach((obj) => {
      obj.traverse((o) => {
        if (o.geometry) o.geometry.dispose();
        if (o.material) o.material.dispose();
      });
    ...
    this.chalkTrailObjects.forEach((trail) =>
      trail.forEach((m) => {
        m.traverse((o) => {
          if (o.geometry) o.geometry.dispose();
          if (o.material) o.material.dispose();
...
// Moved Multisynq.App.makeWidgetDock() to be called before Session.join
